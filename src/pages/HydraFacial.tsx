...
import { ArrowUpRight, Check, Phone, Target, Activity, Zap, ShieldCheck, Clock, Heart, Sparkles } from 'lucide-react';
...
const HydraFacial = () => {
  ...
  const experiencePoints = [
    { text: "Session: 45–60 minutes", icon: <Clock size={16} /> },
    { text: "No pain, no needles, no discomfort", icon: <Heart size={16} /> },
    { text: "No downtime — back to life immediately", icon: <Zap size={16} /> },
    { text: "Skin feels instantly plumper and smoother", icon: <Target size={16} /> },
    { text: "Visible glow immediately after session", icon: <Sparkles size={16} /> },
    { text: "Recommended: monthly sessions", icon: <Clock size={16} /> },
    { text: "Suitable for all skin types and ages", icon: <ShieldCheck size={16} /> }
  ];
    ...
    <div className="bg-brand-cream min-h-screen pt-[104px]">
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        <nav className="text-[10px] md:text-xs uppercase tracking-[0.2em] font-bold text-brand-dark/40">
          ...
          <span className="mx-2 text-brand-gold">›</span>
          <span className="text-brand-dark uppercase tracking-widest">Hydra Facial</span>
        </nav>
      </div>
      ...
      <section className="max-w-7xl mx-auto px-6 pb-24">
        <div className="flex flex-col lg:flex-row gap-16 items-center">
          <div className="w-full lg:w-1/2">
            ...
            >
              Hydra facial — <br /> <span className="text-brand-gold italic font-normal text-5xl md:text-[56px]">plumper, smoother, glowing skin</span>
            </motion.h1>
            <motion.p
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              The Hydra Facial is a multi-step clinical facial that simultaneously cleanses, exfoliates, extracts, and deeply hydrates the skin in a single session. Using patented vortex technology, it delivers professional-grade serums deep into the skin — leaving it instantly plumper, smoother, and visibly glowing with zero downtime.
            </motion.p>
          ...
          <div className="w-full lg:w-1/2">
            <div className="aspect-[4/5] bg-[#E5E1DA] rounded-[24px] overflow-hidden relative border border-brand-muted/30 group">
              ...
              <div className="absolute inset-0 flex items-center justify-center text-brand-dark/10 uppercase tracking-widest text-[10px] font-bold">Hydra Facial Image Placeholder</div>
            </div>
          </div>
        </div>
      </section>
          ...
          <div className="flex flex-col lg:flex-row gap-16">
            <div className="w-full lg:w-1/2 text-white">
              ...
              <h2 className="text-3xl md:text-5xl font-primary font-bold uppercase tracking-tighter leading-tight mb-8 text-white">
                Four treatments. <br /> One session. Zero downtime.
              </h2>
              <p className="font-sans text-[16px] md:text-[18px] text-white/60 leading-[1.7] mb-8">
                The Hydra Facial at Dermalife combines cleansing, exfoliation, extraction, and deep hydration in a single streamlined session — using medical-grade serums customised to your specific skin type and concern. Our specialists select the right serum combination for your skin before every session, ensuring you get targeted results, not a generic treatment.
              </p>
              <div className="flex flex-col sm:flex-row items-center gap-6">
                ...
                <button className="inline-flex items-center gap-2 text-[10px] font-primary font-bold uppercase tracking-widest text-brand-gold group bg-transparent border-none">
                  How it works
            ...
            <div className="w-full lg:w-1/2 flex items-center">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6 w-full">
                {[
                  { text: "Deep cleansing and gentle exfoliation", icon: <Target className="text-brand-gold" size={20} /> },
                  { text: "Painless extraction of congestion", icon: <Activity className="text-brand-gold" size={20} /> },
                  { text: "Intense hydration delivered into skin", icon: <ShieldCheck className="text-brand-gold" size={20} /> },
                  { text: "Instant glow with zero downtime", icon: <Zap className="text-brand-gold" size={20} /> }
                ].map((item, i) => (
      ...
      {/* SECTION 3: EXPERIENCE POINTS (DARK BACKGROUND) */}
      <section className="bg-brand-accent py-16 border-b border-white/5">
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-12 gap-y-8">
            {experiencePoints.map((point, i) => (
              <div key={i} className="flex items-center gap-4 group">
                <div className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center text-brand-gold group-hover:bg-brand-gold group-hover:text-brand-dark transition-all duration-300 shrink-0">
                  {point.icon}
                </div>
                <span className="font-sans text-sm text-white font-medium leading-tight">{point.text}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-24 bg-brand-cream relative overflow-hidden text-center border-t border-brand-muted/30" id="booking-section">
        ...
        <div className="max-w-4xl mx-auto px-6 relative z-10">
          <h2 className="text-3xl md:text-[56px] font-primary font-bold text-brand-dark leading-[1.1] mb-6 uppercase tracking-tighter text-black">
            Want instantly plumper, <br /> <span className="text-brand-gold italic font-normal text-4xl md:text-5xl lg:text-[52px]">smoother, glowing skin?</span>
          </h2>
          <p className="font-sans text-base md:text-lg text-brand-dark/60 mb-12 max-w-2xl mx-auto leading-relaxed">
            Book a free consultation — our specialists will customise your Hydra Facial serum combination for your specific skin type and concern.
          </p>
