...
import { ArrowUpRight, Check, Phone, Target, Sparkles, Activity, Clock } from 'lucide-react';
    ...
    <div className="bg-brand-cream min-h-screen pt-[104px]">
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        <nav className="text-[10px] md:text-xs uppercase tracking-[0.2em] font-bold text-brand-dark/40">
          ...
          <span className="mx-2 text-brand-gold">›</span>
          <span className="text-brand-dark">Lipolasere</span>
        </nav>
      </div>
      ...
      <section className="max-w-7xl mx-auto px-6 pb-24">
        <div className="flex flex-col lg:flex-row gap-16 items-center">
          <div className="w-full lg:w-1/2">
            ...
            >
              Lipolasere — laser fat <br /> <span className="text-brand-gold italic font-normal text-5xl md:text-[56px]">reduction & tightening</span>
            </motion.h1>
            <motion.p
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              Lipolasere uses advanced laser energy to melt stubborn fat deposits while simultaneously tightening the overlying skin. A non-surgical body contouring treatment that targets localised fat in problem areas — delivering visible slimming and sculpting results without any recovery time.
            </motion.p>
          ...
          <div className="w-full lg:w-1/2">
            <div className="aspect-[4/5] bg-[#E5E1DA] rounded-[24px] overflow-hidden relative border border-brand-muted/30">
              <div className="absolute inset-0 flex items-center justify-center text-brand-dark/10 uppercase tracking-widest text-[10px] font-bold">Lipolasere Image Placeholder</div>
            </div>
          </div>
        </div>
      </section>
          ...
          <div className="flex flex-col lg:flex-row gap-16">
            <div className="w-full lg:w-1/2 text-white">
              ...
              <h2 className="text-3xl md:text-5xl font-primary font-bold uppercase tracking-tighter leading-tight mb-8 text-white">
                Laser precision. <br /> Real inch loss.
              </h2>
              <p className="font-sans text-[16px] md:text-[18px] text-white/60 leading-[1.7] mb-8">
                Lipolasere energy levels and treatment parameters must be calibrated to each patient's fat thickness, skin laxity, and target area. At Dermalife, every Lipolasere treatment is assessed and supervised by our certified dermatologists to ensure safe, targeted fat reduction with maximum contouring results.
              </p>
            ...
            <div className="w-full lg:w-1/2 flex items-center">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6 w-full">
                {[
                  { text: "Stubborn localised fat melted", icon: <Target className="text-brand-gold" size={20} /> },
                  { text: "Skin tightened simultaneously", icon: <Sparkles className="text-brand-gold" size={20} /> },
                  { text: "Visible sculpting and contouring", icon: <Activity className="text-brand-gold" size={20} /> },
                  { text: "No surgery, no needles, no downtime", icon: <Clock className="text-brand-gold" size={20} /> }
                ].map((item, i) => (
      ...
      <section className="py-24 bg-brand-cream relative overflow-hidden text-center border-t border-brand-muted/30" id="booking-section">
        ...
        <div className="max-w-4xl mx-auto px-6 relative z-10">
          <h2 className="text-3xl md:text-[56px] font-primary font-bold text-brand-dark leading-[1.1] mb-6 uppercase tracking-tighter text-black">
            Want to melt stubborn fat <br /> <span className="text-brand-gold italic font-normal text-5xl md:text-[56px]">and tighten your skin?</span>
          </h2>
          <p className="font-sans text-base md:text-lg text-brand-dark/60 mb-12 max-w-2xl mx-auto">
            Book a free consultation — our dermatologist will assess your target areas and design the right Lipolasere protocol.
          </p>
